...
class Service:
    def __init__(self):
        ...
        self.auth = OAuth1(CONSUMER_KEY, CONSUMER_SECRET_KEY, ACCESS_TOKEN, ACCESS_TOKEN_SECRET)
    ...
    def init_df(self):
        ...
        if os.path.isfile(CSV_PATH.replace('.csv', '_cleaned.csv')):
            return pd.read_csv(CSV_PATH.replace('.csv', '_cleaned.csv'), skip_blank_lines=True, lineterminator='\n')
    ...
    def fetch(self, tokens):
        ...
        return self.fetch_from_df(tokens=tokens)
        ...
        while required_tokens_count > 0 and len(matched_tweets) < TRASH_HOLD:
            ...
            for author, text in self.df.values:
                try:
                    ...
                    if sum(token in tweet_tokens for token in tokens) >= required_tokens_count:
